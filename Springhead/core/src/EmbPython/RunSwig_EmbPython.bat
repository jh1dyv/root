...
::
::	実行するプロジェクトは ..\..\src\RunSwig\do_swigall.projs に定義されて
::      いるものを使用する. ただしプロジェクト Base は定義の有無に関わりなく
::      実行する.
::
:: *******************************************************************************
::  Version:
...
::	Ver 3.0  2018/12/03 F.Kanehori  Visual Studio 2017 対応
:: *******************************************************************************
...
if "%1" equ "15.0" (
    where /Q nmake.exe
    if %ERRORLEVEL% == 0 (
    	for /F "tokens=*" %%p in ('where nmake.exe') do set MAKEPATH=%%p
	call :dirpart "!MAKEPATH!"
	set MAKEPATH=!$string!
    )
)
