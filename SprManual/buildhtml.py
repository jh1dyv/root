...
parser.add_option('-C', '--copy-only', dest='copy_only',
			action='store_true', default=False,
...
imgsrc = 'images'
imgdst = '%s/%s/images' % (wrkspace, htmls)
...
# (2.5) Kludge
#	lwarpmk html を実行すると何故か漢字で始まる行があったときにエラーを起こす。
#	    pdfTeX error: pdflatex,exe (file cyberb30): Font cyberb30 at 420 not found
#	おまじないとして、空行以外の先頭にダミーのvruleを挿入しておく。
#
cmnd = 'cmd /c add_sp.bat'
if verbose:
	print('#### %s' % cmnd)
