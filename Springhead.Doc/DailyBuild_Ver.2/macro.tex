% macro.tex
%	Last update: 2018/06/11 F.Kanehori
...
\def\endParams{\end{longtable}\end{narrow}\vspace{-.6\baselineskip}}
\def\Param#1#2{%
...
%   \Item{column width(*)}{last column}		(*) without last column
%   \MItem{num-columns}{column spec}{data}
% \end{Table}
...
\def\Item#1#2{%
	\hbox to \Table@name@wid{\tt{#1}}
	&\begin{minipage}[t]{\Table@desc@wid}#2

	\vspace{.2\baselineskip}\end{minipage}\\}
\def\MItem#1#2#3{\multicolumn{#1}{#2}{#3}}
