...
\def\PathB#1{\hbox{\,\raise.4ex\hbox{{\tt "}}\tt{#1}}}
\def\PathE#1{\hbox{\tt{#1}\raise.4ex\hbox{{\tt "}}\,}}
\def\EnvVar#1{\tt{\$#1}}	% for environment variable name (preceded by '$')
...
\def\CmndOpts{\@ifnextchar[{\CmndOpts@}{\CmndOpts@[6em]}}
\def\CmndOpts@[#1]{%
	\noindent options \small{(\,\NOTE はデフォルトを示す)}\\
	\vspace{-1.6\baselineskip}
	\def\CmndOpts@name@wid{#1}
	\def\CmndOpts@desc@wid{\linewidth}
	\addtolength{\CmndOpts@desc@wid}{-\CmndOpts@name@wid}
	\addtolength{\CmndOpts@desc@wid}{-25pt}
	\begin{narrow}[\WID]\begin{longtable}[r]{p{\CmndOpts@name@wid}l}}
\def\endCmndOpts{\end{longtable}\end{narrow}\vspace{-2.2\baselineskip}}
\def\CmndOpt#1#2{%
	\tt{#1}&\begin{minipage}[t]{\CmndOpts@desc@wid}#2

	\vspace{.2\baselineskip}\end{minipage}\\}

\def\CmndArgs{\@ifnextchar[{\CmndArgs@}{\CmndArgs@[6em]}}
\def\CmndArgs@[#1]{\noindent 引数\vspace{-1.6\baselineskip}\\
	\def\CmndArgs@name@wid{#1}
	\def\CmndArgs@desc@wid{\linewidth}
	\addtolength{\CmndArgs@desc@wid}{-\CmndArgs@name@wid}
	\addtolength{\CmndArgs@desc@wid}{-25pt}
	\begin{narrow}[\WID]\begin{longtable}[r]{p{\CmndArgs@name@wid}l}}
\def\endCmndArgs{\end{longtable}\end{narrow}\vspace{-1.7\baselineskip}}
\def\CmndArg#1#2{%
	\tt{#1}&\begin{minipage}[t]{\CmndArgs@desc@wid}#2
...
%	\noindent\GeneralBox@typeface{#3}\begin{narrow}[#2]}
%\def\endGeneralBox{\end{narrow}}

%
%% aliases for program/class
\def\Args{\GeneralBox[b]{引数}}
\let\endArgs\endGeneralBox
\def\Desc{\GeneralBox[b]{概要}}
\let\endDesc\endGeneralBox
\def\Proc{\GeneralBox[b]{処理}}
\let\endProc\endGeneralBox
% aliases for method/function
\def\MArgs{\GeneralBox[m]{引数}}
\let\endMArgs\endGeneralBox
\def\MDesc{\GeneralBox[m]{概要}}
\let\endMDesc\endGeneralBox
\def\MProc{\GeneralBox[m]{処理}}
\let\endMProc\endGeneralBox
