...
\def\Path#1{\tt{\,"#1"\,}}	% for path string
\def\Func#1#2{\tt{\,#1(#2)\,}}	% for function name and args
...
\def\Initializer#1{%	in-line '\\' is ok
	...
	\begin{narrow}[20pt]\vspace{5pt}\tt{\framebox[\linewidth][l]{%
	\begin{tabular}{@{\hspace{5pt}}l}#1\end{tabular}}}
	\vspace{-.7\baselineskip}}
\def\endInitializer{\end{narrow}}

\def\Methods#1{\bigskip\noindent\bf{#1}\begin{narrow}[5pt]\begin{enumerate}}
\def\endMethods{\end{enumerate}\end{narrow}}
\def\Method#1#2{\item\tt{#1(#2)}\begin{narrow}[0pt]}
\def\endMethod{\end{narrow}}

\def\General{\bf{概要}\begin{narrow}[20pt]}
\def\endGeneral{\end{narrow}}

\def\Process{\bf{処理}\begin{narrow}[20pt]}
\def\endProcess{\end{narrow}}

\def\Params{\@ifnextchar[{\Params@}{\Params@[6em]}}
\def\Params@[#1]{\medskip\begin{narrow}[0pt]
	\def\Args@name@wid{#1}
	\def\Args@desc@wid{\linewidth}
	\addtolength{\Args@desc@wid}{-\Args@name@wid}
	\addtolength{\Args@desc@wid}{-25pt}
	\bf{引数}

	\vspace{.2\baselineskip}
	\begin{tabular}{p{\Args@name@wid}l}}
\def\endParams{\end{tabular}\end{narrow}\vspace{.5\baselineskip}}
\def\Param#1#2{%
	\tt{#1}&\begin{minipage}[t]{\Args@desc@wid}#2

	\vspace{.2\baselineskip}\end{minipage}\\}
\def\ParamLong#1#2{%
	\multicolumn{2}{l}{\tt{#1}}\\
	&\begin{minipage}[t]{\Args@desc@wid}#2
