...
\begin{enumerate}
  ...
  \item	比較するレビジョン(new--revision)の決定\\
	...
	\begin{minipage}[t]{\linewidth}
		...
		\small{\phantom{※}\Hskip{1em}(\Ref{subsubsec}{VersionInfo})。}\\
  ...
  \item	\EnvVar{OLDREV}と\EnvVar{NEWREV}のログファイルの取得\\
	...
	\Cmnd{> git show \it{revision:url} | perl \EnvVar{ORDER} > \it{tmpfle}}

  \item	\label{subsubsec:MakeReport:Diff}
	差分の作成\\
	\EnvVar{OLDREV}と\EnvVar{NEWREV}との差分をとる。\\
	\Cmnd{> perl \EnvVar{MYDIFF} \it{tmpfile1} \it{tmpfile2}
