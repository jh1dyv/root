...
\begin{Method}{(\BS @buff1, \BS @buff2) = drop_thread_number(@buff)}
    \label{filter:dropthreadnumber}
    \begin{Args}[r]
	\Item[t]{@buff}{\tt{mydiff()}が出力した差分情報}
    \end{Args}
    \begin{Return}[r]
	\Item{\BS @buff1}{diffマークを取り除いた三つ組情報\\
		(差分記号\Note , 配列要素番号, diffマーク以降の文字列)}
	\Item{\BS @buff2}{スレッド番号までを取り除いた三つ組情報\\
		(差分記号\Note , 配列要素番号, スレッド番号以降の文字列)}
    \end{Return}
    {\small
    \begin{narrow}[10pt][\WID]
	...
	\begin{narrow}[s][\WID]
		\begin{Table}[t][6em]{ll}
		  \Item{1文字目}{差分記号 (意味)}
		  \Item{\tt{'<'} 又は \tt{'-'}}{\tt{'L'}\Hskip{5pt}(削除された行)}
		  \Item{\tt{'>'} 又は \tt{'+'}}{\tt{'R'}\Hskip{5pt}(入された行)}
		  \Item{\tt{'!'}}{\tt{'C'}\Hskip{5pt}(更された行)}
		  \Item{それ以外}{\tt{'-'}\Hskip{5pt}(更のない行)}
		\end{Table}
	\end{narrow}
	このサブルーチンが返すのは、
	\begin{narrow}[s][\WID]
    ...
    \end{Brief}
\end{Method}
\begin{Method}{\$found = find(\$mark, \$linse, \$indx, \BS @buff)}
    \label{filter:find}
    \begin{Args}[r]
	\Item[t]{\$mark}{処理の対象とするdiffマーク}
	\Item[t]{\$line}{検索する文字列(行データ)}
	\Item[t]{\$indx}{検索する文字列の\tt{@buff}での要素番号}
	\Item[t]{@buff}{\tt{drop_thread_number()}が返した三つ組情報の配列}
    \end{Args}
    \begin{Return}[r]
	\Item[t]{\$found}{%
