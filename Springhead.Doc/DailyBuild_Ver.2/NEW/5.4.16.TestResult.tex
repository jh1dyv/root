...
\begin{Brief}[b]
	...
	\small{\begin{Table}[r][18em]{|l|l|}\hline
    		\Item[t]{results}{テスト結果情報を累積する配列。}
    		\Item[t]{\HS results[name][RST.ERR]}{　%
			コンパイル・実行以外でのエラーコード}
    		\Item[t]{\HS results[name][RST.SKP]}{　%
			このテストがスキップされたことを示す}
    		\Item[t]{\HS results[name][RST.BLD][p][c]}{　%
			コンパイルの結果コード}
    		\Item[t]{\HS results[name][RST.RUN][p][c]}{　%
			テスト実行の結果コード}
    		\Item[t]{\HS results[name][RST.RUN][RST.EXP]}{　%
			テスト実行の結果コードの予測値}\hline
    		\Item[t]{visited}{上記の\tt{name}の一覧}\hline
	\end{Table}}
	\vspace{-.5\baselineskip}
...
\begin{Method}{set_info(name, category, info)}
	...
	\Item[t]{category}{情報のカテゴリー(\tt{enum RST})}
...
\begin{Method}{set_result(name, category, platform, config, result)}
	...
	\Item[t]{name}{テスト名(ディレクトリ名)}
	\Item[t]{category}{情報のカテゴリー(\tt{enum RST})}
	\Item[t]{platform}{プラットフォーム(\tt{enum PLATS})}
	\Item[t]{conf}{テスト構成(\tt{enum CONFS})}
