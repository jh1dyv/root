% macro.tex
%	Last update: 2018/07/07 F.Kanehori
...
%% \CmndBox{indent}{command-body}
%  \CmndInline{indent}{command-body}
%
\def\CmndBox#1#2{%
...
\def\CmndInline#1#2{\vbox{%
	\Vskip{.4\baselineskip}\CmndBox{#1}{#2}\Vskip{.1\baselineskip}}}
...
\def\Cmnd{\@ifnextchar[{\Cmnd@}{\Cmnd@[r]}}
\def\Cmnd@[#1]#2{\begin{GeneralBox}[#1]{起動方法}%
	\Vskip{.3\baselineskip}\CmndBox{1em}{#2}\\\end{GeneralBox}}
...
\def\endOpts{\end{Table}\end{narrow}\end{GeneralBox}\Vskip{-.7\baselineskip}}
...
\def\Args@@[#1][#2]{\begin{GeneralBox}[#1]{引数 \isDefault}%
	\begin{narrow}[20pt]\begin{Table}[t][#2]{ll}}
\def\endArgs{\end{Table}\end{narrow}\end{GeneralBox}\Vskip{-.7\baselineskip}}
\def\Brief{\begin{GeneralBox}[t]{概要}}
\def\endBrief{\end{GeneralBox}}
\def\Proc{\begin{GeneralBox}[t]{処理の説明}}
...
\def\GeneralBase{%
	\@ifnextchar[{\GeneralBase@check}{\begin{GeneralBox}{\@title@}}}
\def\GeneralBase@check[#1]{\@ifnextchar[{\GeneralBase@@[#1]}
	{\begin{GeneralBox}[#1][\WID]{\@title@}}}
\def\GeneralBase@@[#1][#2]{\begin{GeneralBox}[#1][#2]{\@title@}}
\def\endGeneralBase{\end{GeneralBox}}
