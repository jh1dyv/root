...
\begin{narrow}[s][15pt]
	\bf{※} この環境変数はデバッグ及びローカルでのテストのときに使用する。
\end{narrow}
...
\begin{enumerate}
  ...
  \item	SpringheadTestにソースツリーを取得する
	\begin{Table}[r][5.2em]{ll}
	    \Item{制御変数名}{\tt{DAILYBUILD_CLEANUP_WORKSPACE}}
	    \Item{処理}{テストディレクトリに最新バージョンをCloneする。}
	\end{Table}
	\vspace{-.7\baselineskip}

  \item	Springheadライブラリを作成する
	\begin{Table}[r][5.2em]{ll}
	    \Item{制御変数名}{DAILYBUILD_EXECUTE_STUBBUILD}
	    \Item{処理}{%
	    	呼出し時の引数及びオプションは次のとおり\\
		\tt{-p x64 -c Release -t 14.0 -C unuse -S} \Cont\\
		\hspace{20pt}\Path{result/dailybuild.result}
		\Path{dailybuild.control}\\
  ...
  \item	\tt{"src/tests"}以下のテストを実行する
	\begin{Table}[r][5.2em]{ll}
	    \Item{制御変数名}{DAILYBUILD_EXECUTE_BUILDRUN}
	    \Item{処理}{%
  ...
  \item	\tt{src/Samples}以下のビルドを実行する
	\begin{Table}[r][5.2em]{ll}
	    \Item{制御変数名}{DAILYBUILD_EXECUTE_SAMPLEBUILD}
	    \Item{処理}{%
	    	呼出し時の引数及びオプションは前項と同じ。
		ただしテスト結果ファイルの初期化は行なわない
		(オプション\tt{-S}なし)}
  ...
  \item	テスト結果ファイルをコミットする
	\begin{Table}[r][5.2em]{ll}
	    \Item{制御変数名}{DAILYBUILD_COMMIT_RESULTOG}
	    \Item{処理}{%
  ...
  \item	履歴ファイルを作成する
	\begin{Table}[r][5.2em]{ll}
	    \Item{制御変数名}{DAILYBUILD_GEN_HISTORY}
	    \Item{処理}{%
  ...
  \item	ログファイルをWebにコピーする
	\begin{Table}[r][5.2em]{ll}
	    \Item{制御変数名}{DAILYBUILD_COPYTO_BUILDLOG}
	    \Item{処理}{テストログファイルをWebウェブサーバにコピーする。}
	    \Item{コピー先}{%
	    	\tt{//haselab/HomeDirs/WWW/docroots/} \Cont\\
		\hspace{40pt}\tt{springhead/daily_build/log} 以下}
	\end{Table}
	\vspace{-.7\baselineskip}

  \item	ドキュメントを作る
	\begin{Table}[r][5.2em]{ll}
	    \Item{制御変数名}{DAILYBUILD_EXECUTE_MAKEDOC}
  ...
  \item	成果物をWebにコピー
	\begin{Table}[r][5.2em]{ll}
	    \Item{制御変数名}{DAILYBUILD_COPYTO_WEBBASE}
	    \Item{処理}{DailyBuildの成果物Webサーバにコピーする}
	    \Item{ファイル}{ライブラリファイル及びドキュメントファイル}
	    \Item{コピー先}{%
	    	\tt{//haselab/HomeDirs/WWW/docroots/} \Cont\\
		\hspace{40pt}\tt{springhead/daily_build/generated} 以下}
	\end{Table}
	\vspace{-.7\baselineskip}
