...
%\begin{CmndOpts}[5em]
\begin{Opts}[5em]
    \Item{-c CONF}{ビルド構成 (\tt{Debug, Release\Note, Trace})}
    \Item{-p PLAT}{プラットフォーム (\tt{x86, x64\Note})}
    \Item{-t TOOL}{ツールセット (\tt{14.0\Note})}
    \Item{-u}{step 1.のみ実行して終了する}
    \Item{-U}{step 2.以降のみ実行する}
    \Item{-A}{step 3.以降のみ実行する}
    \Item{-D}{コマンドを表示するだけで実行はしない(dry run)}
\end{Opts}
