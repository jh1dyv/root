...
\begin{narrow}[20pt]
	...
	white spaceで区切って並べたファイルであり(\tt{Kvfile}クラスで取り扱う)、
	次のような特徴をもつ。
	\begin{itemize}
	  \item	空行は無視される。
...
\bigskip
\begin{Initializer}{%
	obj = ControlParams(fname, section, verbose)}
    \begin{Args}[5em]
...
\begin{Method}{__set_default_path(dic)}
	...
	\begin{enumerate}
	  \item	Springheadのトップディレクトリ ( \RArrow\Hskip{5pt}\tt{dic['SprTop']})
	  \item	Springheadのテストディレクトリ ( \RArrow\Hskip{5pt}\tt{dic['SprTest']})
	\end{enumerate}
    \end{Brief}
\end{Method}
\end{Methods}
