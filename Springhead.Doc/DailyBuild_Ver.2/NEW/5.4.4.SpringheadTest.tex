...
\begin{Args}[b][6em]
    \Item[t]{test-dir}{%
    	テスト対象ディレクトリ\\
	テストを起動するディレクトリから1階層上のディレクトリからの相対パスで指定
	(例えば\Path{core/test}ならば\Path{core}からの相対パス)。}
    \Item[t]{res-file}{%
    	テスト結果累積ファイル名\\
...
\begin{Proc}
	\begin{enumerate}
	  ...
	  \item	\tt{TestResult}クラスのインスタンスを生成して
		...
		起動引数\tt{-S}は指定されず(\tt{scratch=False})、
	  ...
	  \item	\tt{ClosedSrcControl}クラスのインスタンスを生成して、
		...
		実際のファイル名は、\\
		\Hskip{20pt}\Path{core/include/UseClosedSrcOrNot.h}\\
		\Hskip{20pt}\Path{core/test/bin/UseClosedSrc.h.template}\\
		\Hskip{20pt}\Path{core/test/bin/UnuseClosedSrc.h.template}\\
		である。
	  ...
	  \item	\tt{Traverse}クラスのインスタンスを生成して
		...
		\begin{narrow}[n][\WID]\begin{tabular}{l@{\RArrowSp{5pt}}l}
	  ...
	  \item	指定されたディレクトリに対して ビルド＆テストを実行する。\\
		...
		\begin{narrow}[s][\WID]\begin{tabular}{l@{\RArrowSp{5pt}}l}
	  ...
	  \item	テスト結果ファイル\Path{result.log}を作成する。\\
		...
		{\small \let\OrgTabcolsep\tabcolsep \tabcolsep=0pt
		\begin{tabular}{p{12pt}l}
			※ & テストを起動するディレクトリからの相対パスで指定。
		\end{tabular}}
	\end{enumerate}
\end{Proc}
