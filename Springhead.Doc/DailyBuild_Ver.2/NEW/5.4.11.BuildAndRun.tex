...
\begin{Brief}[b]
	...
	コンパイルには、Visual Studio(Windowsの場合)
	又は\tt{/bin/make}(unixの場合)を使用する。
...
\bigskip
\begin{Initializer}{%
	obj = BuildAndRun(ccver, verbose, dry_run)}
    \begin{Args}[5em]
	\Item[t]{ccver}{%
...
\begin{Method}{stat = build(build(dirpath, slnfile, platform, opts, \Cont\\
	...
	\Item[t]{platform}{プラットフォーム(\tt{x86, x64})}
	\Item[t]{opts}{コンパイラオプション\\
			\begin{tabular}{l@{\RArrowSp{5pt}}l}
				Windows & (\tt{Debug, Release, Trace})\\
				unix    & (\tt{-g, -O2})
			\end{tabular}}
...
\begin{Method}{\it{string--list} __select_errors(fname, step)}
    \begin{Brief}[r]
	指定されたファイルを読み、\tt{" error "}を含む行を集めてリストとして返す。
    \end{Brief}
\end{Method}
