...
\Method{diff(\$fh, \%modules1, \$module1, \$base1,
		   \%modules2, \$module2, \$base2)}{%
    \label{mydiff:diff}
    \begin{General}
	モジュール1とモジュール2とを比較して差分情報を作成する。
	比較の対象となるデータ行は、
	\tt{gather\_lines}を用いて\Buff1 と\Buff2 に取り出す
	(\tt{\%modules\it{N}\$module\it{N}]の全体をスレッド順にソートしたもの)。
	\Buff1 と\Buff2 の各行を先頭から順に比較していく。
	...
	\begin{itemize}
	  ...
	  \item	\BuffA の先頭行を\Line1 、\BuffB の先頭行を\LineB とする。
