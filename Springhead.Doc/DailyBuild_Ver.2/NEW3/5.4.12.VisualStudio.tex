...
\begin{Method}{set(func, arg1, arg2)}
	...
	\begin{longtable}[r]{|p{4em}|p{275pt}|}\hline
	    \MC{1}{|c}{\it{func}}
	  ...
	  \TabRow{OUTDIR}{%
		...
		出力ディレクトリ\\をクリアする(強制リコンパイル)。}
	  \TabRow{LOGFILE}{ログファイルパスを\tt{arg1}とする。}
	  \TabRow{DRY_RUN}{%
		Dry_runフラグを\tt{arg1}とする。
		このフラグがTrueならば、
		コマ\\ンドを表示するだけで実行はしない。}
	  \MC{2}{l}{※ \it{func}に与えるのは、このクラスで定義している定数である。}
	\end{longtable}
...
\begin{Method}{get(kind)}
	...
	\begin{longtable}[r]{|p{4em}|p{275pt}|}\hline
	  ...
	  \TabRow{VERSION}{Visual Studioのバージョン (e.g. 14.0)}
	  \TabRow{TOOLSET}{Visual Studio toolset ID (e.g. "v140")}
	  \TabRow{COMMAND}{%
		Visual Studioのtoolset IDと名称\\
		(e.g. "v140" と "Visual Studio 2015")}
	  \MC{2}{l}{※ \it{kind}に与えるのは、このクラスで定義している定数である。}
	\end{longtable}
	\Vskip{-.2\baselineskip}
    \end{Brief}
\end{Method}
