...
%% \Vskip{amount}
%	amount		amount of vertical skip (dimen)
%
...
\def\PushCs#1#2{%
	\def\PushCs@name{\csname\string#1\endcsname}
	\def\PushCs@org{\csname\string Org@#1\endcsname}
	%\let\PushCs@org\PushCs@name
	%\def\PushCs@name{#2}}
	%\def\expandafter\csname Org\string#1\endcsname{\csname#1\endcsname}}
	%\def\csname#1\endcsname{#2}}
	++ \tt{\string \PushCs@name} ++
	++ \tt{\string \PushCs@org} ++}
\def\PopCs#1{%
	\def\PushCs@name{\csname#1\endcsname}
	\def\PushCs@org{\csname Org@#1\endcsname}
	\let\PushCs@name\PushCs@org}

\makeatother

%% test code

\begin{tabular}{|l|}\hline abcde\\\hline\end{tabular}\par

\PushCs{\tabcolsep}{40pt}
%\begin{tabular}{|l|}\hline abcde\\\hline\end{tabular}\par
%\PopCs{tabcolsep}
...
\hrule width 20pt height 3pt
\hrule width 0pt height 10pt

\noindent 最初の行
\begin{Cmnd}{%
	rc = function(arg1, arg2,\\
	\hspace{50pt}arg3, arg4)
}
\end{Cmnd}
\begin{Opts}
