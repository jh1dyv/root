...
\def\Table@@[#1][#2]#3{
	...
	\addtolength{\Table@desc@wid}{-10pt}
	...
	\parskip=-5pt
	\begin{longtable}[#1]{#3}}
%\def\endTable{\end{longtable}\end{LocalScope}\vspace{-1.1\baselineskip}}
\def\endTable{\end{longtable}\end{LocalScope}\vskip -1.5\baselineskip}
...
\def\Item@[#1]#2#3{%
	...
	\hbox to 10pt{}%	% indentation to first column
	\hbox to \Table@name@wid{\Item@type@face{#2}\hfill}
	%&\begin{minipage}[t]{\Table@desc@wid}#3\hfil\end{minipage}\\}
	&{\begin{minipage}[t]{\Table@desc@wid}%
		%\begin{spacing}{.8}#3\hfil\end{spacing}\end{minipage}}\\}
		\begin{spacing}{.8}#3\hfil\end{spacing}\end{minipage}}\\}
\def\ItemT#1#2#3{%
	%\hbox to -6pt{}%	% magic number!
	\hbox to 10pt{}%	% indentation to first column
	\tt{#1}&\tt{#2}
	%&\begin{minipage}[t]{\Table@desc@wid}#3\hfill\par\end{minipage}\\}
	&\begin{minipage}[t]{\Table@desc@wid}#3\hfill\par
	\vspace{.2\baselineskip}\end{minipage}\\}
\let\MC\multicolumn
\def\Cell#1{\begingroup\def\arraystretch{.8}\tabcolsep=0pt%
	\begin{tabular}[t]{l}#1\end{tabular}\endgroup}
