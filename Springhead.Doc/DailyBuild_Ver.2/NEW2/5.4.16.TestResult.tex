...
\begin{Brief}[b]
	...
	\small{\begin{Table}[r][18em]{|l|l|}\hline
    		...
    		\Item[t]{\HS results[\it{name}][RST.ERR]}{　%
			コンパイル・実行以外でのエラーコード}
    		\Item[t]{\HS results[\it{name}][RST.SKP]}{　%
			このテストがスキップされたことを示す}
    		\Item[t]{\HS results[\it{name}][RST.BLD][\it{p}][\it{c}]}{　%
			コンパイルの結果コード}
    		\Item[t]{\HS results[\it{name}][RST.RUN][\it{p}][\it{c}]}{　%
			テスト実行の結果コード}
    		\Item[t]{\HS results[\it{name}][RST.RUN][RST.EXP]}{　%
			テスト実行の結果コードの予測値}\hline
    		\Item[t]{visited}{上記の\tt{\it{name}}の一覧}\hline
	\end{Table}}
	\vspace{-.5\baselineskip}
