...
\begin{Brief}[b]
	...
	\def\TabRow#1#2{\tt{#1\hfil} & \Cell{#2\hfil}\\}
	\Vskip{-.8\baselineskip}
	{\small\begin{longtable}[r]{|p{18em}|p{165pt}|}\hline
    	  \TabRow{results}{テスト結果情報を累積する配列}
    	  \TabRow{\HS results[\it{name}][RST.ERR]}{　%
		コンパイル・実行以外でのエラーコード}
    	  \TabRow{\HS results[\it{name}][RST.SKP]}{　%
		このテストがスキップされたことを示す}
    	  \TabRow{\HS results[\it{name}][RST.BLD][\it{p}][\it{c}]}{　%
		コンパイルの結果コード}
    	  \TabRow{\HS results[\it{name}][RST.RUN][\it{p}][\it{c}]}{　%
		テスト実行の結果コード}
    	  \TabRow{\HS results[\it{name}][RST.RUN][RST.EXP]}{　%
		テスト実行の結果コードの予測値}\hline
    	  \TabRow{visited}{上記の\tt{\it{name}}の一覧}\hline
	\end{longtable}}
